# GMP信息管理系统架构设计文档（补充完善版）

## 🚨 关键设计缺陷修复

### 1. 安全架构设计（新增）

#### 1.1 多层安全策略
```
┌─────────────────────────────────────────────────────────┐
│                网络安全层                               │
│  - 防火墙规则：只允许443端口HTTPS访问                    │
│  - VPN接入：远程访问必须通过VPN                          │
│  - DDoS防护：CloudFlare或阿里云盾                        │
└─────────────────────────────────────────────────────────┘
                            ▼
┌─────────────────────────────────────────────────────────┐
│                应用安全层                               │
│  - JWT Token + Refresh Token                           │
│  - OAuth2.0认证授权                                    │
│  - 输入验证 + SQL注入防护                              │
│  - XSS攻击防护                                         │
└─────────────────────────────────────────────────────────┘
                            ▼
┌─────────────────────────────────────────────────────────┐
│                数据安全层                               │
│  - 数据库连接加密（SSL/TLS）                            │
│  - 敏感数据AES-256加密                                │
│  - 数据传输HTTPS加密                                  │
│  - 密钥管理：HashiCorp Vault                          │
└─────────────────────────────────────────────────────────┘
```

#### 1.2 权限管理设计（RBAC + ABAC）
```java
// 增强的权限管理实体
@Entity
@Table(name = "sys_permissions")
public class Permission {
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;
    
    private String resource;     // 资源：deviation, batch, sample, customer, order, file, message, warehouse, ai_model
    private String action;     // 操作：create, read, update, delete
    private String condition;  // 条件：OWN, DEPARTMENT, ALL
    private String expression; // SpEL表达式：#user.department == #resource.department
}

// 数据权限注解
@Target(ElementType.METHOD)
@Retention(RetentionPolicy.RUNTIME)
public @interface DataPermission {
    String resource();      // 资源类型
    String action();        // 操作类型
}

// 使用示例
@Service
public class DeviationService {
    @DataPermission(resource = "deviation", action = "read")
    public Page<Deviation> findDeviations(Pageable pageable) {
        // 自动根据用户权限过滤数据
        return deviationRepository.findAll(withPermission());
    }
}

// 新增子系统权限示例
@Service
public class CustomerService {
    @DataPermission(resource = "customer", action = "read")
    public Page<Customer> findCustomers(Pageable pageable) {
        return customerRepository.findAll(withPermission());
    }
}

@Service
public class WarehouseService {
    @DataPermission(resource = "warehouse", action = "update")
    public void updateInventory(InventoryUpdateRequest request) {
        // 自动验证用户是否有权限修改库存
        inventoryRepository.updateQuantity(request.getItemId(), request.getQuantity());
    }
}
```

#### 1.3 审计日志设计
```java
// 审计日志实体
@Entity
@Table(name = "audit_logs")
public class AuditLog {
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;
    
    private String userId;          // 用户ID
    private String userName;        // 用户名
    private String action;          // 操作类型
    private String resourceType;    // 资源类型：deviation, batch, sample, customer, order, file, message, inventory, ai_analysis
    private String resourceId;      // 资源ID
    private String oldValue;        // 变更前值（JSON）
    private String newValue;        // 变更后值（JSON）
    private String ipAddress;       // IP地址
    private String userAgent;       // 用户代理
    private String deviceType;      // 设备类型：desktop, mobile, tablet
    private LocalDateTime timestamp; // 操作时间
    private String signature;       // 数字签名防篡改
}

// 审计切面
@Aspect
@Component
public class AuditAspect {
    @Around("@annotation(Auditable)")
    public Object audit(ProceedingJoinPoint joinPoint) throws Throwable {
        // 记录操作前数据
        Object result = joinPoint.proceed();
        // 记录操作后数据并写入审计日志
        writeAuditLog(joinPoint, result);
        return result;
    }
}
```

### 2. 数据完整性保障（新增）

#### 2.1 数据备份策略
```yaml
# 数据库备份配置
backup:
  strategy:
    full_backup:      # 全量备份
      frequency: daily
      time: "02:00"
      retention: 30_days
    
    incremental_backup: # 增量备份
      frequency: hourly
      retention: 7_days
    
    transaction_log_backup: # 事务日志备份
      frequency: 15_minutes
      retention: 24_hours
  
  storage:
    local: /backup/gmp_system
    cloud: s3://gmp-backup-bucket/
    encryption: AES-256
    
  restore:
    test_frequency: weekly
    recovery_time_objective: 4_hours
    recovery_point_objective: 15_minutes
```

#### 2.2 数据一致性校验
```sql
-- 数据一致性检查脚本
CREATE OR REPLACE FUNCTION check_data_consistency()
RETURNS TABLE (
    table_name VARCHAR,
    check_type VARCHAR,
    status VARCHAR,
    message TEXT
) AS $$
BEGIN
    -- 检查外键完整性
    RETURN QUERY
    SELECT 
        tc.table_name::VARCHAR,
        'FOREIGN_KEY'::VARCHAR,
        CASE 
            WHEN COUNT(*) > 0 THEN 'FAILED'
            ELSE 'PASSED'
        END::VARCHAR,
        CONCAT(COUNT(*), ' broken foreign keys')::TEXT
    FROM information_schema.table_constraints tc
    LEFT JOIN information_schema.key_column_usage kcu
        ON tc.constraint_name = kcu.constraint_name
    WHERE tc.constraint_type = 'FOREIGN KEY'
    GROUP BY tc.table_name;
    
    -- 检查数据范围完整性
    RETURN QUERY
    SELECT 
        'deviations'::VARCHAR,
        'DATE_RANGE'::VARCHAR,
        CASE 
            WHEN COUNT(*) > 0 THEN 'FAILED'
            ELSE 'PASSED'
        END::VARCHAR,
        CONCAT(COUNT(*), ' records with invalid dates')::TEXT
    FROM deviations
    WHERE reported_date > CURRENT_DATE 
       OR due_date < reported_date;
END;
$$ LANGUAGE plpgsql;
```

#### 2.3 灾难恢复方案
```bash
#!/bin/bash
# 灾难恢复脚本

# 1. 备份验证
echo "验证备份完整性..."
pg_verifybackup /backup/latest/

# 2. 停止应用服务
echo "停止应用服务..."
docker-compose down

# 3. 恢复数据库
echo "恢复数据库..."
pg_restore -d gmp_system /backup/latest/gmp_database.dump

# 4. 验证数据完整性
echo "验证数据完整性..."
psql -d gmp_system -c "SELECT * FROM check_data_consistency();"

# 5. 启动服务
echo "启动应用服务..."
docker-compose up -d

# 6. 健康检查
echo "系统健康检查..."
curl -f http://localhost:8080/health || exit 1
```

### 3. 性能优化设计（新增）

#### 3.1 性能指标定义
```yaml
# 性能指标配置
performance:
  metrics:
    response_time:
      api_gateway: "< 100ms"
      microservice: "< 200ms"
      database_query: "< 50ms"
    
    throughput:
      concurrent_users: 500
      transactions_per_second: 1000
      file_upload: "10MB/s"
    
    availability:
      uptime: "99.9%"
      mean_time_to_recovery: "< 4 hours"
      mean_time_between_failures: "> 720 hours"
  
  monitoring:
    alerts:
      - name: "High Response Time"
        condition: "response_time > 500ms"
        duration: "5 minutes"
        severity: "warning"
      
      - name: "Database Connection Pool Exhausted"
        condition: "connection_pool_usage > 90%"
        duration: "2 minutes"
        severity: "critical"
```

#### 3.2 缓存策略设计
```java
// 多级缓存配置
@Configuration
@EnableCaching
public class CacheConfig {
    
    @Bean
    public CacheManager cacheManager() {
        // L1缓存：本地Caffeine缓存
        CaffeineCacheManager localCache = new CaffeineCacheManager();
        localCache.setCaffeine(Caffeine.newBuilder()
            .maximumSize(1000)
            .expireAfterWrite(5, TimeUnit.MINUTES));
        
        // L2缓存：Redis分布式缓存
        RedisCacheManager redisCache = RedisCacheManager.builder(redisConnectionFactory())
            .cacheDefaults(RedisCacheConfiguration.defaultCacheConfig()
                .entryTtl(Duration.ofMinutes(30)))
            .withCacheConfiguration("customers", 
                RedisCacheConfiguration.defaultCacheConfig().entryTtl(Duration.ofHours(2)))
            .withCacheConfiguration("products", 
                RedisCacheConfiguration.defaultCacheConfig().entryTtl(Duration.ofHours(1)))
            .withCacheConfiguration("inventory", 
                RedisCacheConfiguration.defaultCacheConfig().entryTtl(Duration.ofMinutes(5)))
            .withCacheConfiguration("ai_models", 
                RedisCacheConfiguration.defaultCacheConfig().entryTtl(Duration.ofDays(1)))
            .build();
        
        // 多级缓存管理器
        return new MultiLevelCacheManager(localCache, redisCache);
    }
}

// 缓存注解使用
@Service
public class DeviationService {
    
    @Cacheable(value = "deviations", key = "#id", unless = "#result == null")
    public Deviation findById(Long id) {
        return deviationRepository.findById(id).orElse(null);
    }
    
    @CacheEvict(value = "deviations", key = "#deviation.id")
    @Auditable(action = "UPDATE")
    public Deviation update(Deviation deviation) {
        return deviationRepository.save(deviation);
    }
}

// 新增子系统缓存使用示例
@Service
public class CustomerService {
    
    @Cacheable(value = "customers", key = "#id")
    public Customer findById(Long id) {
        return customerRepository.findById(id).orElse(null);
    }
    
    @Cacheable(value = "customers", key = "#name")
    public List<Customer> findByName(String name) {
        return customerRepository.findByNameContaining(name);
    }
    
    @CacheEvict(value = "customers", key = "#customer.id")
    @Auditable(action = "UPDATE")
    public Customer update(Customer customer) {
        return customerRepository.save(customer);
    }
}

@Service
public class WarehouseService {
    
    @Cacheable(value = "inventory", key = "#itemId")
    public Inventory getItemInventory(Long itemId) {
        return inventoryRepository.findByItemId(itemId);
    }
    
    @CacheEvict(value = "inventory", key = "#request.itemId")
    @Auditable(action = "UPDATE")
    public void updateInventory(InventoryUpdateRequest request) {
        inventoryRepository.updateQuantity(request.getItemId(), request.getQuantity());
    }
}
```

#### 3.3 数据库性能优化
```sql
-- 性能优化索引
-- 偏差查询优化
CREATE INDEX idx_deviations_status_date ON deviations(status, reported_date);
CREATE INDEX idx_deviations_severity ON deviations(severity) WHERE status != 'CLOSED';

-- 批次查询优化
CREATE INDEX idx_batches_product_status ON production_batches(product_id, status);
CREATE INDEX idx_batches_date ON production_batches(start_time DESC, end_time);

-- 样品查询优化
CREATE INDEX idx_samples_batch ON samples(batch_id) WHERE status = 'TESTING';
CREATE INDEX idx_samples_due_date ON samples(due_date) WHERE status IN ('RECEIVED', 'TESTING');

-- 新增子系统索引
-- CRM索引
CREATE INDEX idx_customers_name ON customers(name);
CREATE INDEX idx_customers_contact ON customers(contact_person, contact_phone);
CREATE INDEX idx_orders_customer_date ON orders(customer_id, order_date DESC);
CREATE INDEX idx_orders_status ON orders(status);

-- 仓库管理索引
CREATE INDEX idx_inventory_item_location ON inventory(item_id, location_id);
CREATE INDEX idx_inventory_quantity ON inventory(quantity);
CREATE INDEX idx_transactions_item_date ON inventory_transactions(item_id, transaction_date DESC);
CREATE INDEX idx_transactions_type ON inventory_transactions(transaction_type);

-- 文件服务索引
CREATE INDEX idx_files_name ON files(file_name);
CREATE INDEX idx_files_upload_date ON files(upload_date DESC);
CREATE INDEX idx_files_type ON files(file_type);
CREATE INDEX idx_files_category ON files(category);

-- 消息服务索引
CREATE INDEX idx_messages_user_status ON messages(user_id, status);
CREATE INDEX idx_messages_sent_date ON messages(sent_date DESC);
CREATE INDEX idx_messages_type ON messages(message_type);

-- AI分析索引
CREATE INDEX idx_ai_analyses_type ON ai_analyses(analysis_type);
CREATE INDEX idx_ai_analyses_date ON ai_analyses.created_date DESC;
CREATE INDEX idx_ai_analyses_status ON ai_analyses(status);

-- 分区表设计（大数据量）
CREATE TABLE audit_logs_2024 PARTITION OF audit_logs
    FOR VALUES FROM ('2024-01-01') TO ('2025-01-01');

-- 物化视图（复杂查询优化）
CREATE MATERIALIZED VIEW batch_quality_summary AS
SELECT 
    pb.batch_number,
    pb.product_name,
    pb.status,
    COUNT(s.id) as sample_count,
    COUNT(CASE WHEN tr.result_status = 'PASS' THEN 1 END) as pass_count,
    COUNT(CASE WHEN tr.result_status = 'FAIL' THEN 1 END) as fail_count
FROM production_batches pb
LEFT JOIN samples s ON pb.id = s.batch_id
LEFT JOIN test_results tr ON s.id = tr.sample_id
GROUP BY pb.id, pb.batch_number, pb.product_name, pb.status;
```

### 4. 系统集成设计（新增）

#### 4.1 新增子系统集成架构
```
┌─────────────────────────────────────────────────────────┐
│                  API网关层                              │
│  - 请求路由与负载均衡                                   │
│  - 统一认证与授权                                       │
│  - API版本控制                                         │
│  - 请求限流与熔断                                       │
└───────────────────────────────────┬─────────────────────┘
                                    │
┌────────────┬────────────┬─────────▼─────────┬────────────┬────────────┐
│   QMS      │   MES      │      LIMS         │   EDMS     │   新增子系统   │
└────────────┴────────────┴─────────┬─────────┴────────────┴────────────┘
                                    │
┌───────────────────────────────────▼───────────────────────────────────┐
│                       服务集成总线(ESB)                               │
│  - 消息队列: Kafka/RabbitMQ                                          │
│  - 事件总线                                                          │
│  - 服务注册发现: Eureka/Nacos                                        │
└───────────────────────────────────┬───────────────────────────────────┘
                                    │
┌────────────────┬──────────┬───────▼───────┬─────────────┬─────────────────┐
│ 外部ERP系统    │ 设备PLC  │  外部CRM系统   │  供应商系统  │  第三方物流系统  │
└────────────────┴──────────┴───────────────┴─────────────┴─────────────────┘
```

#### 4.2 ERP系统集成
```java
// ERP系统适配器
@Component
public class ErpIntegrationAdapter {
    
    @Value("${erp.system.url}")
    private String erpUrl;
    
    @Value("${erp.system.api-key}")
    private String apiKey;
    
    // 同步物料主数据
    public void syncMaterialMaster() {
        List<Material> materials = erpClient.getMaterials();
        materials.forEach(this::updateLocalMaterial);
    }
    
    // 同步生产订单
    public void syncProductionOrders() {
        List<ProductionOrder> orders = erpClient.getProductionOrders();
        orders.forEach(order -> {
            ProductionBatch batch = convertToBatch(order);
            mesService.createBatch(batch);
        });
    }
    
    // 回传质量结果
    public void uploadQualityResults(String batchNumber, List<TestResult> results) {
        QualityReport report = buildQualityReport(batchNumber, results);
        erpClient.uploadQualityReport(report);
    }
}

// 集成配置
@Configuration
public class IntegrationConfig {
    
    @Bean
    public IntegrationFlow erpIntegrationFlow() {
        return IntegrationFlows.from("erpInputChannel")
            .handle("erpIntegrationAdapter", "syncMaterialMaster")
            .channel("erpOutputChannel")
            .get();
    }
    
    @Bean
    public IntegrationFlow warehouseIntegrationFlow() {
        return IntegrationFlows.from("warehouseInputChannel")
            .handle("warehouseIntegrationService", "syncInventory")
            .channel("warehouseOutputChannel")
            .get();
    }
    
    @Bean
    public IntegrationFlow crmIntegrationFlow() {
        return IntegrationFlows.from("crmInputChannel")
            .handle("crmIntegrationService", "syncCustomerData")
            .channel("crmOutputChannel")
            .get();
    }
    
    @Scheduled(fixedRate = 300000) // 5分钟同步一次
    public void scheduleErpSync() {
        erpIntegrationAdapter.syncMaterialMaster();
    }
    
    @Scheduled(fixedRate = 600000) // 10分钟同步一次
    public void scheduleWarehouseSync() {
        warehouseIntegrationService.syncInventory();
    }
    
    @Scheduled(fixedRate = 900000) // 15分钟同步一次
    public void scheduleCrmSync() {
        crmIntegrationService.syncCustomerData();
    }
}
```

#### 4.2 设备PLC接口
```java
// PLC通信服务
@Service
public class PlcCommunicationService {
    
    private final ModbusTcpMaster plcMaster;
    
    @PostConstruct
    public void init() {
        // 初始化Modbus连接
        ModbusTcpConfig config = new ModbusTcpConfig.Builder("192.168.1.100")
            .setPort(502)
            .setTimeout(5000)
            .build();
        this.plcMaster = new ModbusTcpMaster(config);
    }
    
    // 读取设备参数
    public EquipmentData readEquipmentData(int equipmentId) {
        try {
            // 读取温度
            float temperature = plcMaster.readHoldingRegister(1001, 1)[0] / 10.0f;
            
            // 读取压力
            float pressure = plcMaster.readHoldingRegister(1002, 1)[0] / 100.0f;
            
            // 读取运行状态
            boolean running = plcMaster.readCoil(2001);
            
            return EquipmentData.builder()
                .equipmentId(equipmentId)
                .temperature(temperature)
                .pressure(pressure)
                .running(running)
                .timestamp(LocalDateTime.now())
                .build();
                
        } catch (Exception e) {
            log.error("PLC通信失败: {}", e.getMessage());
            throw new PlcCommunicationException("设备数据采集失败");
        }
    }
    
    // 写入控制参数
    public void writeControlParameter(int address, float value) {
        int registerValue = (int)(value * 10);
        plcMaster.writeHoldingRegister(address, registerValue);
    }
}

// OPC UA客户端（现代设备）
@Service
public class OpcUaClientService {
    
    private OpcUaClient opcClient;
    
    @PostConstruct
    public void init() throws Exception {
        EndpointDescription endpoint = DiscoveryClient.getEndpoints("opc.tcp://192.168.1.101:4840")
            .get(0);
            
        OpcUaClientConfig config = OpcUaClientConfig.builder()
            .setEndpoint(endpoint)
            .setIdentityProvider(new UsernameProvider("admin", "password"))
            .build();
            
        this.opcClient = new OpcUaClient(config);
        opcClient.connect().get();
    }
    
    // 订阅设备数据变化
    public void subscribeEquipmentData(String nodeId, Consumer<DataValue> callback) {
        try {
            NodeId node = NodeId.parse(nodeId);
            
            opcClient.subscribe(100.0, (value) -> {
                callback.accept(value);
            }, node);
            
        } catch (Exception e) {
            log.error("OPC UA订阅失败: {}", e.getMessage());
        }
    }
}
```

#### 4.3 API版本管理
```java
// API版本控制配置
@Configuration
public class ApiVersionConfig implements WebMvcConfigurer {
    
    @Override
    public void configurePathMatch(PathMatchConfigurer configurer) {
        configurer.addPathPrefix("/api/v{version}", HandlerTypePredicate.forAnnotation(RestController.class));
    }
}

// 版本注解
@Target({ElementType.METHOD, ElementType.TYPE})
@Retention(RetentionPolicy.RUNTIME)
public @interface ApiVersion {
    String[] value();
}

// 控制器版本控制
@RestController
@RequestMapping("/deviations")
@ApiVersion({"1.0", "1.1"})
public class DeviationController {
    
    @GetMapping
    @ApiVersion("1.0")
    public ApiResponse<List<Deviation>> getDeviationsV1() {
        // v1.0版本逻辑
        return ApiResponse.success(deviationService.findAll());
    }
    
    @GetMapping
    @ApiVersion("1.1")
    public ApiResponse<Page<Deviation>> getDeviationsV1_1(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size) {
        // v1.1版本支持分页
        Pageable pageable = PageRequest.of(page, size);
        return ApiResponse.success(deviationService.findAll(pageable));
    }
}
```

### 5. 监控告警设计（新增）

#### 5.1 新增子系统监控指标

| 子系统名称 | 核心监控指标 | 告警阈值 | 严重性 |
|---------|------------|--------|--------|
| CRM销售系统 | 订单处理响应时间 | > 1s | 警告 |
| | 订单创建失败率 | > 5% | 严重 |
| | 客户数据同步延迟 | > 30分钟 | 警告 |
| 文件服务 | 文件上传/下载速度 | < 1MB/s | 警告 |
| | 存储空间使用率 | > 80% | 警告 |
| | 文件访问错误率 | > 3% | 严重 |
| 消息服务 | 消息发送延迟 | > 5秒 | 警告 |
| | 消息队列积压数量 | > 1000 | 严重 |
| | 消息送达成功率 | < 99.5% | 警告 |
| 移动应用平台 | API调用响应时间 | > 2s | 警告 |
| | 移动端连接数 | 异常波动 > 20% | 警告 |
| | 登录失败率 | > 10% | 严重 |
| 仓库管理系统 | 库存更新延迟 | > 10秒 | 警告 |
| | 库存盘点差异率 | > 2% | 警告 |
| | 批次追踪失败率 | > 1% | 严重 |
| AI智能分析平台 | 模型推理时间 | > 5s | 警告 |
| | 分析任务队列长度 | > 50 | 警告 |
| | 模型准确率下降 | > 5% | 严重 |

#### 5.2 应用监控
```yaml
# 应用监控配置
management:
  endpoints:
    web:
      exposure:
        include: health,metrics,prometheus,auditevents
  
  metrics:
    export:
      prometheus:
        enabled: true
    
    distribution:
      percentiles-histogram:
        http.server.requests: true
      slo:
        http.server.requests: 50ms,100ms,200ms,500ms
  
  health:
    diskspace:
      threshold: 1GB
    db:
      validation-query: SELECT 1
    redis:
      timeout: 2s
```

#### 5.2 业务监控
```java
// 业务指标监控
@Component
public class BusinessMetrics {
    
    private final MeterRegistry meterRegistry;
    
    // 业务指标
    private final Counter deviationCounter;
    private final Timer batchProcessTimer;
    private final Gauge overdueDeviationGauge;
    
    // 新增子系统业务指标
    private final Counter orderCounter;
    private final Timer orderProcessTimer;
    private final Gauge inventoryLevelGauge;
    private final Counter fileAccessCounter;
    private final Timer aiModelInferenceTimer;
    private final Counter messageDeliveryCounter;
    
    public BusinessMetrics(MeterRegistry meterRegistry) {
        this.meterRegistry = meterRegistry;
        
        this.deviationCounter = Counter.builder("gmp.deviations.created")
            .description("偏差创建数量")
            .register(meterRegistry);
            
        this.batchProcessTimer = Timer.builder("gmp.batch.process.time")
            .description("批次处理时间")
            .register(meterRegistry);
            
        this.overdueDeviationGauge = Gauge.builder("gmp.deviations.overdue")
            .description("逾期偏差数量")
            .register(meterRegistry, this, BusinessMetrics::getOverdueDeviationCount);
            
        // 新增CRM指标
        this.orderCounter = Counter.builder("gmp.orders.created")
            .description("订单创建数量")
            .register(meterRegistry);
            
        this.orderProcessTimer = Timer.builder("gmp.order.process.time")
            .description("订单处理时间")
            .register(meterRegistry);
            
        // 新增仓库指标
        this.inventoryLevelGauge = Gauge.builder("gmp.inventory.level")
            .description("库存水平")
            .register(meterRegistry, this, BusinessMetrics::getInventoryLevel);
            
        // 新增文件服务指标
        this.fileAccessCounter = Counter.builder("gmp.file.access")
            .description("文件访问次数")
            .tag("type", "download")
            .register(meterRegistry);
            
        // 新增AI服务指标
        this.aiModelInferenceTimer = Timer.builder("gmp.ai.model.inference.time")
            .description("AI模型推理时间")
            .register(meterRegistry);
            
        // 新增消息服务指标
        this.messageDeliveryCounter = Counter.builder("gmp.message.delivery")
            .description("消息送达数量")
            .tag("status", "success")
            .register(meterRegistry);
    }
    
    public void recordDeviationCreated() {
        deviationCounter.increment();
    }
    
    public void recordBatchProcessTime(Duration duration) {
        batchProcessTimer.record(duration);
    }
    
    private double getOverdueDeviationCount() {
        return deviationRepository.countOverdueDeviations();
    }
    
    // 新增子系统监控方法
    public void recordOrderCreated() {
        orderCounter.increment();
    }
    
    public void recordOrderProcessTime(Duration duration) {
        orderProcessTimer.record(duration);
    }
    
    public void recordFileAccess(String accessType) {
        Counter.builder("gmp.file.access")
            .tag("type", accessType)
            .register(meterRegistry)
            .increment();
    }
    
    public void recordAIModelInferenceTime(Duration duration) {
        aiModelInferenceTimer.record(duration);
    }
    
    public void recordMessageDelivery(boolean success) {
        Counter.builder("gmp.message.delivery")
            .tag("status", success ? "success" : "failure")
            .register(meterRegistry)
            .increment();
    }
    
    private double getInventoryLevel() {
        return inventoryService.getAverageInventoryLevel();
    }
}
```

#### 5.3 告警规则
```yaml
# Prometheus告警规则
groups:
  - name: gmp_system_alerts
    rules:
      - alert: HighErrorRate
        expr: rate(http_requests_total{status=~"5.."}[5m]) > 0.1
        for: 5m
        labels:
          severity: critical
        annotations:
          summary: "高错误率告警"
          description: "{{ $labels.service }} 错误率超过10%"
      
      - alert: HighResponseTime
        expr: histogram_quantile(0.95, rate(http_request_duration_seconds_bucket[5m])) > 0.5
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "响应时间过长"
          description: "{{ $labels.service }} 95%响应时间超过500ms"
      
      - alert: DatabaseConnectionPoolLow
        expr: hikaricp_connections_active / hikaricp_connections_max > 0.9
        for: 2m
        labels:
          severity: critical
        annotations:
          summary: "数据库连接池不足"
          description: "{{ $labels.pool }} 连接池使用率超过90%"
      
      - alert: OverdueDeviations
        expr: gmp_deviations_overdue > 5
        for: 1m
        labels:
          severity: warning
        annotations:
          summary: "逾期偏差过多"
          description: "有{{ $value }}个偏差已逾期"
      
      # 新增子系统告警规则
      - alert: CrmOrderProcessingSlow
        expr: histogram_quantile(0.95, rate(gmp_order_process_time_seconds_bucket[5m])) > 1
        for: 5m
        labels:
          severity: warning
          service: crm-service
        annotations:
          summary: "CRM订单处理缓慢"
          description: "订单处理95%响应时间超过1秒"
      
      - alert: CrmOrderCreationFailure
        expr: rate(gmp_orders_creation_failure_total[5m]) / rate(gmp_orders_creation_attempts_total[5m]) > 0.05
        for: 5m
        labels:
          severity: critical
          service: crm-service
        annotations:
          summary: "CRM订单创建失败率高"
          description: "订单创建失败率超过5%"
      
      - alert: FileStorageNearCapacity
        expr: node_filesystem_size_bytes{mountpoint="/app/storage"} - node_filesystem_free_bytes{mountpoint="/app/storage"} > 0.8 * node_filesystem_size_bytes{mountpoint="/app/storage"}
        for: 10m
        labels:
          severity: warning
          service: file-service
        annotations:
          summary: "文件存储接近容量上限"
          description: "文件存储使用率超过80%"
      
      - alert: MessageQueueBacklog
        expr: kafka_consumergroup_lag{group="message-service-consumer"} > 1000
        for: 5m
        labels:
          severity: critical
          service: message-service
        annotations:
          summary: "消息队列积压严重"
          description: "消息队列积压消息数超过1000条"
      
      - alert: WarehouseInventoryUpdateSlow
        expr: histogram_quantile(0.95, rate(warehouse_inventory_update_time_seconds_bucket[5m])) > 0.01
        for: 5m
        labels:
          severity: warning
          service: warehouse-service
        annotations:
          summary: "仓库库存更新缓慢"
          description: "库存更新95%响应时间超过10秒"
      
      - alert: AIModelInferenceSlow
        expr: histogram_quantile(0.95, rate(gmp_ai_model_inference_time_seconds_bucket[5m])) > 5
        for: 5m
        labels:
          severity: warning
          service: ai-service
        annotations:
          summary: "AI模型推理缓慢"
          description: "AI模型推理95%响应时间超过5秒"
```

### 6. 部署架构优化（新增）

#### 6.1 容器化部署优化

##### 6.1.1 通用服务基础镜像
```dockerfile
# 优化的Dockerfile
FROM openjdk:17-jre-slim

# 安全加固
RUN apt-get update && apt-get install -y \
    curl \
    && rm -rf /var/lib/apt/lists/* \
    && addgroup --system gmp \
    && adduser --system --group gmp

# 创建应用目录
WORKDIR /app

# 复制JAR文件
COPY target/*.jar app.jar

# 健康检查
HEALTHCHECK --interval=30s --timeout=3s --start-period=60s --retries=3 \
    CMD curl -f http://localhost:8080/actuator/health || exit 1

# 使用非root用户运行
USER gmp

# JVM优化参数
ENV JAVA_OPTS="-Xms512m -Xmx1024m -XX:+UseG1GC -XX:+UseStringDeduplication"

EXPOSE 8080

ENTRYPOINT ["sh", "-c", "java $JAVA_OPTS -jar app.jar"]
```

##### 6.1.2 文件服务镜像
```dockerfile
FROM openjdk:17-jre-slim

# 安装必要工具
RUN apt-get update && apt-get install -y \
    curl \
    ffmpeg \
    && rm -rf /var/lib/apt/lists/* \
    && addgroup --system gmp \
    && adduser --system --group gmp

# 创建应用和存储目录
WORKDIR /app
RUN mkdir -p /app/storage && chown -R gmp:gmp /app/storage

# 复制JAR文件
COPY target/*.jar app.jar

# 健康检查
HEALTHCHECK --interval=30s --timeout=3s --start-period=60s --retries=3 \
    CMD curl -f http://localhost:8088/actuator/health || exit 1

# 使用非root用户运行
USER gmp

# JVM优化参数 - 针对文件服务增加内存
ENV JAVA_OPTS="-Xms1g -Xmx2g -XX:+UseG1GC -XX:+UseStringDeduplication"

EXPOSE 8088
VOLUME ["/app/storage"]

ENTRYPOINT ["sh", "-c", "java $JAVA_OPTS -jar app.jar"]
```

##### 6.1.3 AI服务镜像
```dockerfile
FROM python:3.9-slim

# 安装必要工具和Python依赖
RUN apt-get update && apt-get install -y \
    curl \
    gcc \
    libgomp1 \
    && rm -rf /var/lib/apt/lists/* \
    && addgroup --system gmp \
    && adduser --system --group gmp

# 创建应用目录
WORKDIR /app

# 复制Python依赖文件
COPY requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

# 复制应用代码
COPY . .

# 健康检查
HEALTHCHECK --interval=30s --timeout=5s --start-period=120s --retries=3 \
    CMD curl -f http://localhost:8092/health || exit 1

# 使用非root用户运行
USER gmp

# 环境变量
ENV MODEL_DIR="/app/models" \
    LOG_LEVEL="INFO"

EXPOSE 8092

ENTRYPOINT ["gunicorn", "--bind", "0.0.0.0:8092", "app:app", "--workers", "4", "--timeout", "300"]
```
```

#### 6.2 Kubernetes部署配置

##### 6.2.1 CRM服务部署配置
```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: gmp-crm-service
  labels:
    app: gmp-crm-service
spec:
  replicas: 3
  selector:
    matchLabels:
      app: gmp-crm-service
  template:
    metadata:
      labels:
        app: gmp-crm-service
    spec:
      containers:
      - name: gmp-crm-service
        image: gmp/crm-service:latest
        ports:
        - containerPort: 8089
        env:
        - name: SPRING_PROFILES_ACTIVE
          value: "prod"
        - name: DB_HOST
          value: "postgres-service"
        - name: REDIS_HOST
          value: "redis-service"
        resources:
          requests:
            memory: "512Mi"
            cpu: "500m"
          limits:
            memory: "1Gi"
            cpu: "1000m"
        livenessProbe:
          httpGet:
            path: /actuator/health/liveness
            port: 8089
          initialDelaySeconds: 60
          periodSeconds: 30
        readinessProbe:
          httpGet:
            path: /actuator/health/readiness
            port: 8089
          initialDelaySeconds: 30
          periodSeconds: 10
---
apiVersion: v1
kind: Service
metadata:
  name: gmp-crm-service
spec:
  selector:
    app: gmp-crm-service
  ports:
  - port: 80
    targetPort: 8089
  type: ClusterIP
```

##### 6.2.2 文件服务部署配置
```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: gmp-file-service
  labels:
    app: gmp-file-service
spec:
  replicas: 2
  selector:
    matchLabels:
      app: gmp-file-service
  template:
    metadata:
      labels:
        app: gmp-file-service
    spec:
      containers:
      - name: gmp-file-service
        image: gmp/file-service:latest
        ports:
        - containerPort: 8088
        env:
        - name: SPRING_PROFILES_ACTIVE
          value: "prod"
        - name: DB_HOST
          value: "postgres-service"
        - name: STORAGE_PATH
          value: "/app/storage"
        resources:
          requests:
            memory: "1Gi"
            cpu: "500m"
          limits:
            memory: "2Gi"
            cpu: "1000m"
        volumeMounts:
        - name: file-storage
          mountPath: /app/storage
        livenessProbe:
          httpGet:
            path: /actuator/health/liveness
            port: 8088
          initialDelaySeconds: 60
          periodSeconds: 30
        readinessProbe:
          httpGet:
            path: /actuator/health/readiness
            port: 8088
          initialDelaySeconds: 30
          periodSeconds: 10
      volumes:
      - name: file-storage
        persistentVolumeClaim:
          claimName: file-storage-pvc
---
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
  name: file-storage-pvc
spec:
  accessModes:
    - ReadWriteMany
  resources:
    requests:
      storage: 100Gi
---
apiVersion: v1
kind: Service
metadata:
  name: gmp-file-service
spec:
  selector:
    app: gmp-file-service
  ports:
  - port: 80
    targetPort: 8088
  type: ClusterIP
```

##### 6.2.3 AI服务部署配置
```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: gmp-ai-service
  labels:
    app: gmp-ai-service
spec:
  replicas: 2
  selector:
    matchLabels:
      app: gmp-ai-service
  template:
    metadata:
      labels:
        app: gmp-ai-service
    spec:
      containers:
      - name: gmp-ai-service
        image: gmp/ai-service:latest
        ports:
        - containerPort: 8092
        env:
        - name: MODEL_DIR
          value: "/app/models"
        - name: LOG_LEVEL
          value: "INFO"
        - name: DB_HOST
          value: "postgres-service"
        resources:
          requests:
            memory: "2Gi"
            cpu: "1000m"
          limits:
            memory: "4Gi"
            cpu: "2000m"
        volumeMounts:
        - name: model-storage
          mountPath: /app/models
        livenessProbe:
          httpGet:
            path: /health
            port: 8092
          initialDelaySeconds: 120
          periodSeconds: 30
        readinessProbe:
          httpGet:
            path: /health
            port: 8092
          initialDelaySeconds: 60
          periodSeconds: 10
      volumes:
      - name: model-storage
        persistentVolumeClaim:
          claimName: model-storage-pvc
---
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
  name: model-storage-pvc
spec:
  accessModes:
    - ReadWriteOnce
  resources:
    requests:
      storage: 50Gi
---
apiVersion: v1
kind: Service
metadata:
  name: gmp-ai-service
spec:
  selector:
    app: gmp-ai-service
  ports:
  - port: 80
    targetPort: 8092
  type: ClusterIP
```

##### 6.2.4 消息服务部署配置
```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: gmp-message-service
  labels:
    app: gmp-message-service
spec:
  replicas: 2
  selector:
    matchLabels:
      app: gmp-message-service
  template:
    metadata:
      labels:
        app: gmp-message-service
    spec:
      containers:
      - name: gmp-message-service
        image: gmp/message-service:latest
        ports:
        - containerPort: 8087
        env:
        - name: SPRING_PROFILES_ACTIVE
          value: "prod"
        - name: DB_HOST
          value: "postgres-service"
        - name: KAFKA_BROKERS
          value: "kafka:9092"
        - name: SMTP_HOST
          valueFrom:
            secretKeyRef:
              name: email-credentials
              key: smtp-host
        - name: SMTP_USERNAME
          valueFrom:
            secretKeyRef:
              name: email-credentials
              key: smtp-username
        - name: SMTP_PASSWORD
          valueFrom:
            secretKeyRef:
              name: email-credentials
              key: smtp-password
        resources:
          requests:
            memory: "512Mi"
            cpu: "500m"
          limits:
            memory: "1Gi"
            cpu: "1000m"
        livenessProbe:
          httpGet:
            path: /actuator/health/liveness
            port: 8087
          initialDelaySeconds: 60
          periodSeconds: 30
        readinessProbe:
          httpGet:
            path: /actuator/health/readiness
            port: 8087
          initialDelaySeconds: 30
          periodSeconds: 10
---
apiVersion: v1
kind: Service
metadata:
  name: gmp-message-service
spec:
  selector:
    app: gmp-message-service
  ports:
  - port: 80
    targetPort: 8087
  type: ClusterIP
```
```

#### 6.3 服务网格配置（Istio）
```yaml
# 虚拟服务配置
apiVersion: networking.istio.io/v1beta1
kind: VirtualService
metadata:
  name: gmp-qms-service
spec:
  hosts:
  - gmp-qms-service
  http:
  - match:
    - headers:
        api-version:
          exact: v1.0
    route:
    - destination:
        host: gmp-qms-service
        subset: v1
  - match:
    - headers:
        api-version:
          exact: v1.1
    route:
    - destination:
        host: gmp-qms-service
        subset: v1-1
        
---
# 目标规则配置
apiVersion: networking.istio.io/v1beta1
kind: DestinationRule
metadata:
  name: gmp-qms-service
spec:
  host: gmp-qms-service
  trafficPolicy:
    loadBalancer:
      simple: LEAST_CONN
    connectionPool:
      tcp:
        maxConnections: 100
      http:
        http1MaxPendingRequests: 50
    circuitBreaker:
      consecutiveErrors: 3
      interval: 30s
      baseEjectionTime: 30s
  subsets:
  - name: v1
    labels:
      version: v1.0
  - name: v1-1
    labels:
      version: v1.1
```

## 📊 设计缺陷总结与修复建议

### 🔴 严重缺陷
1. **安全架构缺失** → ✅ 已补充完整的多层安全设计
2. **数据备份恢复缺失** → ✅ 已设计完整的备份策略
3. **性能监控不足** → ✅ 已添加详细的性能指标和监控
4. **系统集成方案缺失** → ✅ 已提供ERP和PLC集成方案

### 🟡 一般缺陷
1. **API版本管理缺失** → ✅ 已实现版本控制机制
2. **缓存策略不明确** → ✅ 已设计多级缓存架构
3. **部署方案不完整** → ✅ 已优化容器化和K8s部署
4. **监控告警不全面** → ✅ 已完善业务和技术监控

### 💡 额外优化建议
1. **考虑使用Service Mesh**（如Istio）进行微服务治理
2. **引入事件溯源**（Event Sourcing）保证数据可追溯性
3. **使用CQRS模式**分离读写操作，提升性能
4. **实现蓝绿部署**减少系统停机时间
5. **AI服务性能优化**：考虑GPU加速和模型量化技术提升AI服务性能
6. **移动应用适配**：实现PWA（Progressive Web App）技术，提升移动端体验
7. **数据湖集成**：将仓库管理和销售数据集成到数据湖中，支持更复杂的数据分析
8. **API网关增强**：添加API版本管理、请求限流和监控能力

### 📋 新增子系统部署清单

| 子系统名称 | 服务名称 | 端口 | 资源需求 | 数据存储 | 关键监控指标 |
|---------|--------|-----|---------|---------|------------|
| CRM销售系统 | gmp-crm-service | 8089 | 1GB内存, 1CPU | PostgreSQL | 订单处理时间, 订单失败率 |
| 文件服务 | gmp-file-service | 8088 | 2GB内存, 1CPU | PostgreSQL + PV (100GB) | 文件上传/下载速度, 存储空间使用率 |
| 消息服务 | gmp-message-service | 8087 | 1GB内存, 1CPU | PostgreSQL + Kafka | 消息发送延迟, 消息队列积压数 |
| 移动应用平台 | gmp-mobile-service | 8090 | 1GB内存, 1CPU | PostgreSQL | API响应时间, 移动连接数 |
| 仓库管理系统 | gmp-warehouse-service | 8091 | 1GB内存, 1CPU | PostgreSQL | 库存更新延迟, 批次追踪成功率 |
| AI智能分析平台 | gmp-ai-service | 8092 | 4GB内存, 2CPU | PostgreSQL + PV (50GB) | 模型推理时间, 分析任务队列长度 |

这些补充设计确保了系统的：**安全性**、**可靠性**、**性能**和**可维护性**，满足GMP系统的严格要求，并为新增的6个子系统提供了完整的架构支持。

这些补充设计确保了系统的：**安全性**、**可靠性**、**性能**和**可维护性**，满足GMP系统的严格要求。