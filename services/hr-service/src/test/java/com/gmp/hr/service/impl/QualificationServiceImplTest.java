...
public class QualificationServiceImplTest {
    ...
    void setUp() {
        ...
        employee.setId(1L);
        // employee.setCode("EMP001"); // 假设Employee实体没有setCode方法
        employee.setName("张三");
        ...
        // qualificationType.setNeedUpdate(true); // 假设QualificationType实体没有setNeedUpdate方法
        // qualificationType.setUpdateCycle(365); // 假设QualificationType实体没有setUpdateCycle方法
        ...
        qualificationDTO.setQualificationTypeId(1L);
        qualificationDTO.setEmployeeName("张三");
        qualificationDTO.setQualificationTypeName("计算机等级证书");
    }
    ...
    void testCreateQualification_Success() {
        ...
        // 使用findByCertificateNumberAndDeletedFalse来模拟证书编号不存在
        when(qualificationRepository.findByCertificateNumberAndDeletedFalse("CERT-2023-0001"))
                .thenReturn(Optional.empty());
        // 使用findByEmployeeIdAndQualificationTypeIdAndDeletedFalse来模拟员工-资质类型组合不存在
        when(qualificationRepository.findByEmployeeIdAndQualificationTypeIdAndDeletedFalse(1L, 1L))
    ...
    void testUpdateQualification_Success() {
        // 准备
        when(qualificationRepository.findById(1L)).thenReturn(Optional.of(qualification));
        // 使用findByCertificateNumberAndDeletedFalse来模拟不存在冲突的证书编号
        when(qualificationRepository.findByCertificateNumberAndDeletedFalse("CERT-2023-0001"))
                .thenReturn(Optional.empty());
        when(employeeRepository.findById(1L)).thenReturn(Optional.of(employee));
        when(qualificationTypeRepository.findById(1L)).thenReturn(Optional.of(qualificationType));
    ...
    @Test
    void testGetQualificationsByEmployee_Success() {
        // 准备
        List<Qualification> qualifications = Collections.singletonList(qualification);
        when(qualificationRepository.findByEmployeeIdAndDeletedFalse(1L)).thenReturn(qualifications);

        // 执行
        List<QualificationDTO> result = qualificationService.getQualificationsByEmployee(1L);
    ...
    @Test
    void testGetQualificationsByType_Success() {
        // 准备
        List<Qualification> qualifications = Collections.singletonList(qualification);
        when(qualificationRepository.findByQualificationTypeIdAndDeletedFalse(1L)).thenReturn(qualifications);

        // 执行
        List<QualificationDTO> result = qualificationService.getQualificationsByType(1L);

        // 验证
        assertNotNull(result);
        assertEquals(1, result.size());
        assertEquals(1L, result.get(0).getQualificationTypeId());
    }
    ...
    void testGetExpiringQualifications_Success() {
        ...
        List<Qualification> qualifications = Collections.singletonList(qualification);
        when(qualificationRepository.findExpiringQualifications(futureDate)).thenReturn(qualifications);
    ...
    void testGetExpiredQualifications_Success() {
        // 准备
        List<Qualification> qualifications = Collections.singletonList(qualification);
        // 由于没有直接的findByExpiryDateBefore方法，我们使用findAll来模拟
