...
public class DocumentVersionTest {
    ...
    public void testVersionBasicProperties() {
        ...
        assertEquals("user123", version.getUploaderId());
        assertEquals("初始版本", version.getDescription());
        assertTrue(version.getIsMajor());
        assertEquals("ACTIVE", version.getStatus());
        assertEquals("admin", version.getCreatedBy());
        assertEquals("admin", version.getUpdatedBy());
        assertFalse(version.getIsDeleted());
        assertEquals("{\"converted\": true}", version.getExtraInfo());
    }
    ...
    public void testFormatFileSize() {
        ...
        assertEquals("100 B", version.formatFileSize());
        
        // 测试KB级别的大小
        version.setFileSize(1024L);
        assertEquals("1.0 KB", version.formatFileSize());
        
        version.setFileSize(1536L); // 1.5KB
        assertEquals("1.5 KB", version.formatFileSize());
        
        // 测试MB级别的大小
        version.setFileSize(1024L * 1024); // 1MB
        assertEquals("1.0 MB", version.formatFileSize());
        
        version.setFileSize(1024L * 1536); // 1.5MB
        assertEquals("1.5 MB", version.formatFileSize());
        
        // 测试GB级别的大小
        version.setFileSize(1024L * 1024 * 1024); // 1GB
        assertEquals("1.0 GB", version.formatFileSize());
        
        version.setFileSize(1024L * 1024 * 1536); // 1.5GB
        assertEquals("1.5 GB", version.formatFileSize());
        
        // 测试0大小
        version.setFileSize(0L);
        assertEquals("0 B", version.formatFileSize());
    ...
    public void testSetters() {
        ...
        updatedVersion.setIsMajor(false);
        updatedVersion.setStatus("INACTIVE");
        updatedVersion.setIsDeleted(true);
        ...
        assertFalse(updatedVersion.getIsMajor());
        assertEquals("INACTIVE", updatedVersion.getStatus());
        assertTrue(updatedVersion.getIsDeleted());
    }
    ...
    public void testEdgeCaseFileSizes() {
        ...
        assertEquals("1023 B", version.formatFileSize());
        ...
        assertEquals("1.0 TB", version.formatFileSize());
    }
}
